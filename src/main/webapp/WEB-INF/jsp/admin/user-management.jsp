...
<head>
    ...
    <style>
        ...
        .content-card { border-radius: 20px; border: none; box-shadow: 0 4px 6px rgba(0,0,0,0.05); background: #ffffff; padding: 30px; }
        .modal-xl-custom { max-width: 800px; }
    </style>
</head>
<body>
        ...
        <div class="col-md-10">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h4 class="fw-bold m-0">
                    <i class="fa-solid fa-users-gear text-primary me-2"></i> Registered Users Management
                </h4>
                <a href="${pageContext.request.contextPath}/dashboard" class="btn btn-outline-secondary rounded-pill">
                    <i class="fa-solid fa-arrow-left me-1"></i> Back to Dashboard Sidebar
                </a>
            </div>
            ...
            <div class="content-card mb-5">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <button type="button" class="btn btn-danger btn-sm rounded-pill px-3 fw-semibold" data-bs-toggle="modal" data-bs-target="#bannedListModal">
                            <i class="fa-solid fa-user-slash me-1"></i> View Banned List
                        </button>
                        <span class="badge bg-primary rounded-pill px-3 py-2 fs-7">
                            Total: ${not empty users ? users.size() : 0}
                        </span>
                    </div>
                </div>
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            <c:choose>
                                <c:when test="${not empty users}">
                                    <c:forEach items="${users}" var="user" varStatus="count">
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge ${user.role eq 'admin' ? 'bg-primary' : (user.status eq 'suspended' ? 'bg-danger' : 'bg-secondary')}">
                                                    ${user.status eq 'suspended' ? 'Suspended' : user.role}
                                                </span>
                                            </td>
                                            <td class="text-center">
                                                <c:choose>
                                                    <c:when test="${user.userId != sessionScope.loginUser.userId && user.status != 'suspended'}">
                                                    ...
                                                    <c:when test="${user.status == 'suspended'}">
                                                        <span class="text-danger small fw-semibold"><i class="fa-solid fa-user-lock me-1"></i> Suspended</span>
                                                    </c:when>
                                                    <c:otherwise>
                                ...
                                <c:otherwise>
                                    <tr>
                                        <td colspan="4" class="text-center text-muted py-4">No active system users found.</td>
                                    </tr>
                                </c:otherwise>
                            </c:choose>
                        </tbody>
...
<div class="modal fade" id="bannedListModal" tabindex="-1" aria-labelledby="bannedListModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl-custom">
        <div class="modal-content" style="border-radius: 20px; border: none; box-shadow: 0 10px 40px rgba(0,0,0,0.2); border-top: 5px solid #dc3545;">
            <div class="modal-header border-0 pt-4 px-4">
                <h5 class="modal-title fw-bold text-danger" id="bannedListModalLabel">
                    <i class="fa-solid fa-user-slash me-2"></i> Banned Accounts List
                </h5>
                <span class="badge bg-danger rounded-pill px-3 py-2 ms-3">
                    Banned Total: ${not empty bannedUsers ? bannedUsers.size() : 0}
                </span>
                                ...
                                <c:otherwise>
                                    <tr>
                                        <td colspan="3" class="text-center text-muted py-4">
                                            လက်ရှိတွင် ပိတ်ပင်ထားသော အသုံးပြုသူ မရှိပါ။
                                        </td>
                                    </tr>
                                </c:otherwise>
                            </c:choose>
...
<div class="modal fade" id="reasonModal" tabindex="-1" aria-labelledby="reasonModalLabel" aria-hidden="true" style="z-index: 1060;">
            ...
            <form action="${pageContext.request.contextPath}/admin/user/ban" method="POST">
                <div class="modal-header border-0 pt-4 px-4">
                    <h5 class="modal-title fw-bold text-danger" id="banModalLabel">
                        <i class="fa-solid fa-user-lock me-2"></i> Ban User: <span id="targetUsername" class="text-dark"></span>
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body px-4">
					...
					<div class="mb-3">
    					<label for="banDuration" class="form-label fw-semibold text-secondary">Ban Duration</label>
    					<select class="form-select" id="banDuration" name="banDuration" style="border-radius: 12px;" required>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="reason" name="reason" rows="3" required style="border-radius: 12px;" placeholder="Please provide a valid reason..."></textarea>
                    </div>
                </div>
                <div class="modal-footer border-0 pb-4 px-4 gap-2">
                    <button type="button" class="btn btn-light px-4 rounded-pill" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-danger px-4 rounded-pill fw-bold">Confirm Ban</button>
                </div>
            </form>
        </div>
    </div>
